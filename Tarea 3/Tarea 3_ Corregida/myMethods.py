...
#Correción del método de Newton-Raphson
def mNR1(f, df, x0, maxIter=1000, TOL=1e-10):
    """
    Método de Newton-Raphson mejorado con manejo robusto de errores.
    ...
    if not callable(f) or not callable(df):
        raise ValueError("f y df deben ser funciones callable")
    ...
    try:
        for i in range(maxIter):
            ...
            # Guardar historial
            history.append(x)
            fn_values.append(abs(fx))
            
            # Criterios de parada
            if abs(fx) < TOL:
                return x, i+1, history, fn_values
            
            # Verificar derivada cero
            if abs(dfx) < 1e-15:
                # Intentar pequeño desplazamiento
                dfx = df(x + 1e-10)
                if abs(dfx) < 1e-15:
                    raise ZeroDivisionError("Derivada practicamente cero")
            ...
            if abs(x_new) > 1e10 and i > 5:
                raise ValueError("Posible divergencia detectada")
            
            # Criterio de convergencia basado en cambio relativo
            if abs(delta) < TOL * max(1.0, abs(x_new)):
                return x_new, i+1, history, fn_values
            
            x = x_new
        
        # Si llegamos aquí, no convergió en maxIter
        return x, maxIter, history, fn_values
        
    except ZeroDivisionError:
        return x, i, history, fn_values
