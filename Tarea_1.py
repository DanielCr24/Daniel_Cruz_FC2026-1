...
def descomposition(n):
    ...
    '''
    Recibe un entero positivo n y genera una cadena de su descomposición en factores de potencias de primos.

    Args:
        n (int): Entero a descomponer.

    Returns:
        (str): Cadena con la descomposición de n como factores de potencias de primos.
    '''
    ...
    while n > 1:
        ...
        while n % d == 0:
            ...
            n //= d
        if p > 0:
            pFactors.append(f"{d}^{p}")
        d += 1
    return " * ".join(pFactors)

#Problema 3.#

def nameFile(simName, *args, **kwargs):
            ...
            if isinstance(arg, (list, tuple)):
                for valor in arg:
                    if isinstance(valor, (int, float)):
                        parts.append(str(valor).replace('.', '_'))
                    else:
